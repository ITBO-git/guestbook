...
pipeline {
    ...
    environment {
        ...
        strDockerImage ="itbodocker/cicd_guestbook:${strDockerTag}"
    }

    stages {
        stage('Checkout') {
            steps {
                git branch: 'master', url:'https://github.com/ITBO-git/guestbook'

                dir('/root/sub-workspace/guestbook-config'){
                    git branch: 'master', url:'https://github.com/ITBO-git/guestbook-config'
                }
            }
        }
            ...
            steps {
                dir('/root/sub-workspace/guestbook-config'){
                    sh '''
                        ...
                        git push "https://ITBO-git:${GITHUB_ACCESS_TOKEN}@github.com/ITBO-git/guestbook-config"
                    '''
                }
            }
        }
